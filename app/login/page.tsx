...
export default function LoginPage() {
  ...
  const { signInWithGoogle, signInWithTwitter, isAuthenticated, loading: authLoading } = useAuth();
  const [loading, setLoading] = useState<'google' | 'twitter' | null>(null);
  const [error, setError] = useState<string | null>(null);

  // If already authenticated, redirect to connect using useEffect
  useEffect(() => {
      ...
      >
        ...
        <div className="space-y-3 mb-6">
          <motion.button
            ...
            className="w-full flex items-center justify-center gap-3 px-6 py-3 bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white border border-zinc-200 dark:border-zinc-800 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium"
          >
            {loading === 'google' ? (
          ...
          >
            ...
            {loading === 'twitter' ? 'Signing in...' : 'Sign in with Twitter'}
          </motion.button>
        </div>
